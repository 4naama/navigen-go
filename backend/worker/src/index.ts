...
function normalizeInitiationType(v: unknown): string {
  const s = String(v || "").trim().toLowerCase();
  if (s === "agent" || s === "platform") return s;
...
async function createCampaignCheckoutSession(env: Env, req: Request, body: any, noStore: Record<string, string>) {
  ...
  const initiationType = normalizeInitiationType(body?.initiationType); // owner | agent | platform; legacy public maps to owner
  ...
  // Allow both owner-initiated and public legacy initiation types; persistence normalizes public to owner.
  // Canonical product: Plan payment is the state transition; session is minted on return.
  const okInitiation = (initiationType === "owner" || initiationType === "agent" || initiationType === "platform");
