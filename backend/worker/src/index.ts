...
// --- Partner-3 reservation stake checkout helpers: separate from Plan checkout
type PartnerLeadReservationStatus = "draft" | "checkout_created" | "paid" | "converted_to_lead" | "expired" | "void";

type PartnerLeadReservationRecord = {
  ver: number;
  reservationId: string;
  partnerId: string;
  status: PartnerLeadReservationStatus;
  businessName: string;
  website: string;
  phone: string;
  address: string;
...
const PARTNER_RESERVATION_STAKE_AMOUNT_CENTS = 100;
const PARTNER_RESERVATION_STAKE_CURRENCY = "eur";
const PARTNER_RESERVATION_CHECKOUT_TTL_SECONDS = 60 * 60 * 24;

function partnerLeadReservationKey(reservationId: string): string {
  return `partner_lead_reservation:${reservationId}`;
}

function partnerLeadReservationBySessionKey(checkoutSessionId: string): string {
  return `partner_lead_reservation_by_session:${checkoutSessionId}`;
}

function mintPartnerLeadReservationId(): string {
  const bytes = new Uint8Array(18);
  crypto.getRandomValues(bytes);
  return `plres_${bytesToB64url(bytes)}`;
}

function publicPartnerLeadReservation(reservation: PartnerLeadReservationRecord): Record<string, unknown> {
  return {
...
async function readPartnerLeadReservation(env: Env, reservationId: string): Promise<PartnerLeadReservationRecord | null> {
  const id = String(reservationId || "").trim();
  if (!id) return null;

  const reservation = await env.KV_STATUS.get(partnerLeadReservationKey(id), { type: "json" }) as PartnerLeadReservationRecord | null;
  if (!reservation || typeof reservation !== "object" || String(reservation.reservationId || "").trim() !== id) return null;

  return reservation;
}

async function writePartnerLeadReservation(env: Env, reservation: PartnerLeadReservationRecord): Promise<void> {
  await env.KV_STATUS.put(
...
async function readPartnerLeadReservationBySession(env: Env, checkoutSessionId: string): Promise<PartnerLeadReservationRecord | null> {
...
function partnerLeadReservationPayloadFromBody(body: any): {
  businessName: string;
  website: string;
  phone: string;
  address: string;
...
function partnerLeadReservationPayloadHasContactSignal(payload: {
  ...
  return !!(
    normalizePartnerLeadWebsite(payload.website) ||
    normalizePartnerLeadPhone(payload.phone) ||
    normalizePartnerLeadText([payload.address, payload.city, payload.country].filter(Boolean).join(" "), 500)
  );
}

async function finalizePaidPartnerLeadReservationFromSession(
  env: Env,
  session: any
): Promise<{ reservation: PartnerLeadReservationRecord; lead: PartnerLeadRecord | null }> {
  const checkoutSessionId = String(session?.id || "").trim();
  ...
  const reservation = reservationId
    ? await readPartnerLeadReservation(env, reservationId)
    : await readPartnerLeadReservationBySession(env, checkoutSessionId);
  ...
  if (duplicateLeadId) {
    ...
    if (currentDuplicate && (partnerLeadIsOpen(currentDuplicate) || currentDuplicate.status === "converted")) {
      const voided: PartnerLeadReservationRecord = {
        ...reservation,
        status: "void",
        checkoutSessionId,
        paymentIntentId,
  ...
  const lead: PartnerLeadRecord = {
    ...
    createdAt: nowIso,
    updatedAt: nowIso,
    expiresAt: new Date(Date.now() + PARTNER_LEAD_RESERVATION_DAYS * 24 * 60 * 60 * 1000).toISOString()
  };
  ...
  const converted: PartnerLeadReservationRecord = {
    ...reservation,
...
async function handlePartnerReservationCheckoutCreate(req: Request, env: Env): Promise<Response> {
  const auth = await requirePartnerSession(req, env);
  if (auth instanceof Response) return auth;
  ...
  const reservationDraft: PartnerLeadReservationRecord = {
    ver: 1,
    reservationId,
    partnerId: profile.partnerId,
    status: "draft",
    ...
    createdAt: nowIso,
    updatedAt: nowIso,
    expiresAt: new Date(nowMs + PARTNER_RESERVATION_CHECKOUT_TTL_SECONDS * 1000).toISOString()
  };
  ...
  let stripeOut: any = null;
  try { stripeOut = JSON.parse(stripeText); } catch { stripeOut = null; }
  ...
  const reservation: PartnerLeadReservationRecord = {
    ...reservationDraft,
...
async function handlePartnerReservationCheckoutReturn(req: Request, env: Env): Promise<Response> {
  const auth = await requirePartnerSession(req, env);
  if (auth instanceof Response) return auth;
  ...
  const metadata = session?.metadata || {};
  if (
    String(metadata?.flow || "").trim() !== "partner_lead_reservation" ||
    String(metadata?.reservationType || "").trim() !== "lead_stake"
  ) {
    return json(
      {
        error: {
  ...
  let finalized: { reservation: PartnerLeadReservationRecord; lead: PartnerLeadRecord | null } | null = null;
  try {
...
// --- End Partner-3 reservation stake checkout helpers

// --- Partner-4 lead draft preparation helpers: Partner-owned draft editing, no BO authority
function partnerLeadDraftBaseFromLead(lead: PartnerLeadRecord): Record<string, any> {
  return {
...
function publicPartnerLeadDraft(draft: any, draftULID: string): Record<string, unknown> {
...
async function readPartnerOwnedLeadForDraft(
  req: Request,
  env: Env,
  leadId: string,
  opts: { requireReserved?: boolean } = {}
): Promise<{ auth: PartnerSessionResolution; lead: PartnerLeadRecord } | Response> {
  const auth = await requirePartnerSession(req, env);
  if (auth instanceof Response) return auth;
...
async function buildPartnerDraftPatch(
  env: Env,
  body: any,
  base: Record<string, any> = {}
): Promise<Record<string, any> | Response> {
  const rawDraft = (body?.draft && typeof body.draft === "object")
    ? body.draft
...
async function handlePartnerLeadDraftCreate(req: Request, env: Env, leadId: string): Promise<Response> {
  ...
  const existingDraftSessionId = String((lead as any).draftSessionId || "").trim();
  ...
  const nextLead: PartnerLeadRecord = {
    ...lead,
    draftULID,
    draftSessionId,
    updatedAt: nowIso
...
async function handlePartnerLeadDraftRead(req: Request, env: Env, leadId: string): Promise<Response> {
  const resolved = await readPartnerOwnedLeadForDraft(req, env, leadId);
  if (resolved instanceof Response) return resolved;

  const lead = resolved.lead;
  const draftULID = String(lead.draftULID || "").trim();
  const draftSessionId = String((lead as any).draftSessionId || "").trim();
...
async function handlePartnerLeadDraftUpdate(req: Request, env: Env, leadId: string): Promise<Response> {
  const resolved = await readPartnerOwnedLeadForDraft(req, env, leadId, { requireReserved: true });
  if (resolved instanceof Response) return resolved;
  ...
  const draftKey = `override_draft:${draftULID}:${draftSessionId}`;
  const prev = await env.KV_STATUS.get(draftKey, { type: "json" }) as any;
...
// --- End Partner-4 lead draft preparation helpers

// --- Partner-5 handoff token and BO preview helpers: token preview only, no BO authority
type PartnerHandoffStatus = "created" | "viewed" | "accepted" | "expired" | "revoked";

type PartnerHandoffRecord = {
  ver: number;
  handoffId: string;
  tokenHash: string;
  tokenHint: string;
...
const PARTNER_HANDOFF_TOKEN_TTL_SECONDS = 60 * 60 * 24 * 14;

function partnerHandoffKey(tokenHash: string): string {
  return `partner_handoff:${tokenHash}`;
}

function partnerHandoffsByLeadKey(leadId: string): string {
  return `partner_handoffs_by_lead:${leadId}`;
}

function mintPartnerHandoffId(): string {
  const bytes = new Uint8Array(18);
  crypto.getRandomValues(bytes);
  return `phand_${bytesToB64url(bytes)}`;
}

function mintPartnerHandoffToken(): string {
  const bytes = new Uint8Array(32);
  crypto.getRandomValues(bytes);
  return `ph_${bytesToB64url(bytes)}`;
}

function isValidPartnerHandoffToken(token: string): boolean {
  return /^ph_[A-Za-z0-9_-]{32,120}$/.test(String(token || "").trim());
}

function publicPartnerHandoff(handoff: PartnerHandoffRecord): Record<string, unknown> {
  return {
...
async function partnerHandoffTokenHash(token: string): Promise<string> {
  return await sha256Hex(String(token || "").trim());
}

async function readPartnerHandoffByToken(env: Env, token: string): Promise<PartnerHandoffRecord | null> {
  const rawToken = String(token || "").trim();
  if (!isValidPartnerHandoffToken(rawToken)) return null;

  const tokenHash = await partnerHandoffTokenHash(rawToken);
  const handoff = await env.KV_STATUS.get(partnerHandoffKey(tokenHash), { type: "json" }) as PartnerHandoffRecord | null;
...
async function writePartnerHandoff(env: Env, handoff: PartnerHandoffRecord): Promise<void> {
  await env.KV_STATUS.put(
...
async function readPartnerHandoffIndex(env: Env, leadId: string): Promise<string[]> {
...
async function appendPartnerHandoffIndex(env: Env, leadId: string, tokenHash: string): Promise<void> {
...
async function expirePartnerHandoffIfNeeded(env: Env, handoff: PartnerHandoffRecord): Promise<PartnerHandoffRecord> {
  ...
  const next: PartnerHandoffRecord = {
    ...handoff,
    status: "expired",
    updatedAt: new Date().toISOString()
  };
...
async function buildPartnerHandoffPreview(env: Env, handoff: PartnerHandoffRecord): Promise<{
  handoff: PartnerHandoffRecord;
  ...
  const draftSessionId = String((currentLead as any).draftSessionId || "").trim();
...
async function handlePartnerHandoffCreate(req: Request, env: Env, leadId: string): Promise<Response> {
  const resolved = await readPartnerOwnedLeadForDraft(req, env, leadId, { requireReserved: true });
  if (resolved instanceof Response) return resolved;
  ...
  const handoff: PartnerHandoffRecord = {
    ver: 1,
    ...
    createdAt: nowIso,
    updatedAt: nowIso,
    expiresAt: new Date(nowMs + PARTNER_HANDOFF_TOKEN_TTL_SECONDS * 1000).toISOString(),
    viewedAt: "",
    acceptedAt: "",
    acceptedEmail: ""
  };
...
async function handlePartnerHandoffPreview(req: Request, env: Env, token: string): Promise<Response> {
  if (!partnerRoutesEnabled(env)) return partnerDisabledResponse(env);
...
async function handlePartnerHandoffAccept(req: Request, env: Env, token: string): Promise<Response> {
  if (!partnerRoutesEnabled(env)) return partnerDisabledResponse(env);
...
async function markPartnerLeadConvertedAfterPlanReconciliation(
  env: Env,
  session: any,
  reconciled: PlanReconciliationResult,
  logTag: string
): Promise<void> {
  try {
    const meta = (session?.metadata && typeof session.metadata === "object") ? session.metadata : {};
    ...
    if (selectionId) {
      selection = await env.KV_STATUS.get(planSelectionKey(selectionId), { type: "json" }) as any;
    }
...
// --- End Partner-5 handoff token and BO preview helpers

// --- Partner-6 Partner-assisted BO Plan payment helpers: dedicated handoff checkout path
function partnerPlanCheckoutBlockedResponse(env: Env): Response {
  return json(
    {
      error: {
...
function partnerCheckoutSuccessUrl(siteOrigin: string, checkoutSessionPlaceholder: string): string {
...
function partnerCheckoutCancelUrl(siteOrigin: string, token: string): string {
  const cancelUrl = new URL(`/partner/handoff/${encodeURIComponent(token)}`, siteOrigin);
  cancelUrl.searchParams.set("canceled", "1");
  return cancelUrl.toString();
}

function partnerCampaignDraftFromCheckoutBody(body: any, draftULID: string, requestedPlan: PlanDefinition & { code: string; priceId: string }, planMode: PlanMode): any {
  const src = (body?.campaignDraft && typeof body.campaignDraft === "object")
    ? body.campaignDraft
    : body;
...
async function handlePartnerHandoffPlanCheckout(req: Request, env: Env, token: string): Promise<Response> {
  if (!partnerRoutesEnabled(env)) return partnerDisabledResponse(env);
  ...
  const draftSessionId = String((preview.lead as any).draftSessionId || "").trim();
...
// --- End Partner-6 Partner-assisted BO Plan payment helpers

// --- Partner-7 commission ledger helpers: ledger only, no payout or transfer creation
type PartnerCommissionStatus = "pending_requires_connect" | "pending" | "eligible" | "paid" | "void" | "adjusted";
...
type PartnerCommissionRecord = {
  ...
  renewalTailMonths: number;
  totalCapAmount: number;
  totalCapAmountCents: number;
  status: PartnerCommissionStatus;
  stripeConnectedAccountId: string;
  connectStatusAtCreation: PartnerConnectStatus;
  source: string;
  createdAt: string;
  updatedAt: string;
  eligibleAt: string;
...
type PartnerCommissionPolicy = {
  commissionPolicyVersion: string;
  activationBountyAmount: number;
  activationBountyAmountCents: number;
  renewalSharePercent: number;
  renewalTailMonths: number;
  totalCapAmount: number;
  totalCapAmountCents: number;
...
const PARTNER_COMMISSION_ELIGIBILITY_DAYS = 14;
const PARTNER_COMMISSION_INDEX_LIMIT = 500;

function partnerCommissionKey(commissionId: string): string {
  return `partner_commission:${commissionId}`;
}

function partnerCommissionByPaymentKey(paymentIntentId: string): string {
  return `partner_commission_by_payment:${paymentIntentId}`;
}

function partnerCommissionsByPartnerKey(partnerId: string): string {
  return `partner_commissions_by_partner:${partnerId}`;
}

function partnerCommissionsByLeadKey(partnerLeadId: string): string {
  return `partner_commissions_by_lead:${partnerLeadId}`;
}

function mintPartnerCommissionId(): string {
  const bytes = new Uint8Array(18);
  crypto.getRandomValues(bytes);
  return `pc_${bytesToB64url(bytes)}`;
}

function normalizePartnerCommissionStatus(value: unknown): PartnerCommissionStatus | "" {
  const s = String(value || "").trim().toLowerCase();
  if (
    s === "pending_requires_connect" ||
    s === "pending" ||
    s === "eligible" ||
    s === "paid" ||
    s === "void" ||
...
function partnerCommissionPolicyFor(tier: PlanTier, policyVersion: string): PartnerCommissionPolicy | null {
  ...
  if (tier === "standard") {
    return {
      ...
      activationBountyAmountCents: 4000,
      renewalSharePercent: 20,
      renewalTailMonths: 5,
      totalCapAmount: 120,
      totalCapAmountCents: 12000
    };
  }
  ...
  if (tier === "multi") {
    return {
      ...
      activationBountyAmountCents: 9000,
      renewalSharePercent: 20,
      renewalTailMonths: 5,
      totalCapAmount: 270,
      totalCapAmountCents: 27000
    };
  }
...
function publicPartnerCommission(commission: PartnerCommissionRecord): Record<string, unknown> {
  return {
...
async function readPartnerCommission(env: Env, commissionId: string): Promise<PartnerCommissionRecord | null> {
  const id = String(commissionId || "").trim();
  if (!id) return null;

  const commission = await env.KV_STATUS.get(partnerCommissionKey(id), { type: "json" }) as PartnerCommissionRecord | null;
  if (!commission || typeof commission !== "object" || String(commission.commissionId || "").trim() !== id) return null;

  return commission;
}

async function writePartnerCommission(env: Env, commission: PartnerCommissionRecord): Promise<void> {
...
async function readPartnerCommissionIndex(env: Env, key: string): Promise<string[]> {
  const raw = await env.KV_STATUS.get(key, "text");
  let rows: string[] = [];
...
async function appendPartnerCommissionIndex(env: Env, key: string, commissionId: string): Promise<void> {
...
async function listPartnerCommissions(env: Env, partnerId: string): Promise<PartnerCommissionRecord[]> {
...
async function ensurePartnerCommissionAfterPlanReconciliation(
  env: Env,
  session: any,
  reconciled: PlanReconciliationResult,
  logTag: string
): Promise<PartnerCommissionRecord | null> {
  try {
    const meta = (session?.metadata && typeof session.metadata === "object") ? session.metadata : {};
    const initiationType = normalizeInitiationType(meta?.initiationType || reconciled?.plan?.initiationType);
    ...
    const allocation = (reconciled?.allocation && typeof reconciled.allocation === "object")
      ? (reconciled.allocation as any)
      : {};
    ...
    const tier = (reconciled?.plan?.tier || "unknown") as PlanTier;
    ...
    const nowMs = Date.now();
    const nowIso = new Date(nowMs).toISOString();
    const eligibleAt = new Date(nowMs + PARTNER_COMMISSION_ELIGIBILITY_DAYS * 24 * 60 * 60 * 1000).toISOString();
...
async function handlePartnerCommissionList(req: Request, env: Env): Promise<Response> {
  const auth = await requirePartnerSession(req, env);
  if (auth instanceof Response) return auth;
...
// --- End Partner-7 commission ledger helpers

// --- Partner-9 NaviGen Admin Platform Partner module helpers: internal ops only, no BO Dashboard
type AdminPartnerListResult = {
  items: PartnerProfileRecord[];
  cursor: string;
  scanned: number;
};

function adminNoStoreHeaders(): Record<string, string> {
  return { "cache-control": "no-store" };
}

function adminAuthError(req: Request, env: Env): Response | null {
...
function normalizeAdminPartnerStatus(value: unknown): PartnerStatus | "" {
  const s = String(value || "").trim().toLowerCase();
  if (s === "applicant" || s === "verified" || s === "trusted" || s === "regional_partner" || s === "suspended") return s;
  return "";
}

function adminPartnerProfile(profile: PartnerProfileRecord): Record<string, unknown> {
  return {
    partnerId: profile.partnerId,
    status: profile.status,
...
async function listAdminPartnerProfiles(env: Env, opts: {
  cursor?: string;
  limit?: number;
  status?: PartnerStatus | "";
  ...
  const items: PartnerProfileRecord[] = [];
  let scanned = 0;
...
async function handleAdminPartnerList(req: Request, env: Env): Promise<Response> {
...
async function handleAdminPartnerRead(req: Request, env: Env, partnerId: string): Promise<Response> {
  const authError = adminAuthError(req, env);
  if (authError) return authError;
...
async function handleAdminPartnerStatusUpdate(req: Request, env: Env, partnerId: string): Promise<Response> {
  const authError = adminAuthError(req, env);
  if (authError) return authError;
...
async function handleAdminPartnerCapacityUpdate(req: Request, env: Env, partnerId: string): Promise<Response> {
  const authError = adminAuthError(req, env);
  if (authError) return authError;
  ...
  const body = await req.json().catch(() => ({})) as any;
  const nextLeadCapacity = Math.max(0, Math.min(10000, Math.trunc(Number(body?.leadCapacity ?? profile.leadCapacity) || 0)));
  const nextFreeLeadQuota = Math.max(0, Math.min(10000, Math.trunc(Number(body?.freeLeadQuota ?? profile.freeLeadQuota) || 0)));
...
async function handleAdminPartnerRevokeAttribution(req: Request, env: Env, partnerId: string): Promise<Response> {
  const authError = adminAuthError(req, env);
  if (authError) return authError;
  ...
  const changedCommissions: PartnerCommissionRecord[] = [];
  ...
  for (const commissionId of commissionIds) {
    ...
    const nextCommission: PartnerCommissionRecord = {
      ...commission,
...
async function handleAdminPartnerCommissionList(req: Request, env: Env): Promise<Response> {
  const authError = adminAuthError(req, env);
  if (authError) return authError;
  ...
  let commissions: PartnerCommissionRecord[] = [];
  let nextCursor = "";
...
// --- End Partner-9 NaviGen Admin Platform Partner module helpers

// --- Partner-10 Stripe Connect Express helpers: onboarding/status only, no transfers or payout run
type PartnerConnectStatusResult = {
  profile: PartnerProfileRecord;
  account: any | null;
};
...
async function partnerStripeFormRequest(sk: string, path: string, form: URLSearchParams): Promise<any> {
...
async function partnerStripeGetRequest(sk: string, path: string): Promise<any> {
  const resp = await fetch(`https://api.stripe.com${path}`, {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${sk}`
...
function partnerConnectStatusFromStripeAccount(account: any): PartnerConnectStatus {
...
async function promotePartnerCommissionsAfterConnect(env: Env, profile: PartnerProfileRecord): Promise<void> {
  ...
  for (const commission of commissions) {
    ...
    const next: PartnerCommissionRecord = {
      ...commission,
      status: "pending",
      stripeConnectedAccountId: profile.stripeConnectedAccountId,
...
async function refreshPartnerConnectStatus(env: Env, profile: PartnerProfileRecord): Promise<PartnerConnectStatusResult> {
  const sk = String((env as any).STRIPE_SECRET_KEY || "").trim();
  if (!sk) {
  ...
  if (!accountId) {
    if (profile.connectStatus !== "not_started") {
      ...
      await writePartnerProfile(env, next);
      return { profile: next, account: null };
    }
...
async function handlePartnerConnectStart(req: Request, env: Env): Promise<Response> {
  const auth = await requirePartnerSession(req, env);
  if (auth instanceof Response) return auth;
...
async function handlePartnerConnectStatus(req: Request, env: Env): Promise<Response> {
  const auth = await requirePartnerSession(req, env);
  if (auth instanceof Response) return auth;

  let result: PartnerConnectStatusResult;
  try {
...
async function handlePartnerConnectReturn(req: Request, env: Env): Promise<Response> {
...
async function handleStripeWebhook(req: Request, env: Env): Promise<Response> {
  ...
  try {
    const reconciled = await reconcilePaidCheckoutSessionPlan(env, String((env as any).STRIPE_SECRET_KEY || "").trim(), session, { logTag: "stripe_webhook" });
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      // --- Partner reservation stake checkout: dedicated Partner path, not Plan checkout
      ...
      // --- End Partner reservation stake checkout routes
      
      // --- Partner lead draft preparation: Partner-4 backend routes
      ...
      // --- End Partner lead draft preparation routes

      // --- Partner handoff token and BO preview: Partner-5 backend routes
      ...
      // --- End Partner handoff token and BO preview routes      

      // --- Partner-assisted BO Plan payment: Partner-6 backend route
      ...
      // --- End Partner-assisted BO Plan payment route

      // --- Partner commission ledger: Partner-7 backend route
      if (normPath === "/api/partner/commissions" && req.method === "GET") {
        return await handlePartnerCommissionList(req, env);
      }
      
      // --- End Partner commission ledger route

      // --- NaviGen Admin Platform Partner module: Partner-9 backend routes
      ...
      // --- End NaviGen Admin Platform Partner module routes

      // --- Partner Stripe Connect Express: Partner-10 backend routes
